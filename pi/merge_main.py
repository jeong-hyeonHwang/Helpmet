...
async def websocket_handler(request):
    ...
    for pc in list(pcs):
        try:
            print("[RTC] 이전 RTCPeerConnection 종료 중...")
            await pc.close()
        except Exception as e:
            print(f"[RTC] 이전 연결 종료 중 오류: {e}")
        pcs.discard(pc)

    # 필요 시 WebSocket도 정리
#    for ws in list(websockets):
#        try:
#            print("[WS] 기존 WebSocket 종료 중...")
#            await ws.close(code=1000, message='새 연결을 위해 종료됨')
#        except Exception as e:
#            print(f"[WS] WebSocket 종료 실패: {e}")
